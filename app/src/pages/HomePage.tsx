    ...
    <section className="relative w-full h-[calc(100vh-72px)] overflow-hidden mt-[72px]">
      ...
      <div className="relative z-10 h-full flex items-center max-w-[1440px] mx-auto px-6 lg:px-12">
        <div className="max-w-[600px]">
          <h1 className="font-playfair text-5xl md:text-6xl lg:text-7xl font-normal leading-[1.05] text-[#1B2A6B] mb-8 tracking-widest">
            WEAR<br />YOUR<br />JOURNEY!
          </h1>
          <Link
            to="/shop"
            className="font-playfair inline-block bg-[#1B2A6B] text-white text-sm font-normal tracking-widest uppercase px-8 py-4 hover:bg-[#162050] transition-colors"
          >
            Shop Collection
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {categoryTiles.map(c => (
            <Link key={c.name} to={`/shop?category=${c.name}`} className="group block text-center">
              ...
              <span className="font-playfair text-2xl md:text-3xl font-semibold uppercase tracking-widest text-[#1B2A6B]">{c.name}</span>
            </Link>
          ))}
        </div>
      </div>
  ...
  return (
    <section className="bg-[#1A1A1A] text-white">
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-16 md:py-20 grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
        <h2 className="font-playfair text-5xl md:text-6xl lg:text-7xl font-normal tracking-widest text-center leading-[1.4]">
          <span className="block mb-3">FEATURED</span>PRODUCTS!
        </h2>
